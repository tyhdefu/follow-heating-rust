...
mod tests {
    ...
    fn manual_check() {
        ...
        let contained = Utc::from_utc_datetime(&Utc, &NaiveDateTime::new(bst_date, time(22,30,32)));
        assert!(zoned_time_slot.contains(&contained));


        let gmt_date = date(2021, 01, 16); // GMT
        let not_contained = Utc::from_utc_datetime(&Utc, &NaiveDateTime::new(gmt_date, time(22,30,32)));
