...
mod test {
    ...
    fn check_basic() {
        ...
        assert_eq!(model.recommended_temp(time(02, 45, 00)), Some(35.0), "midpoint should be correct");
        assert_eq!(model.recommended_temp(time(03, 37, 30)), Some(42.5), "midpoint should be correct");

        assert_eq!(model.recommended_temp(time(00, 13, 58)), None, "no immersion heater");
        assert_eq!(model.recommended_temp(time(16, 37, 43)), None, "no immersion heater");
        assert_eq!(model.recommended_temp(time(12, 21, 54)), None, "no immersion heater");
    }

    #[test]
    fn check_complification() {
        let model = ImmersionHeaterModelConfig::new(
            vec![
                ImmersionHeaterModelPart::from_time_points((time(00, 30, 00), 20.0), (time(01, 30, 00), 35.0), Sensor::TKTP),
                ImmersionHeaterModelPart::from_time_points((time(01, 20, 00), 35.0), (time(04, 30, 00), 50.0), Sensor::TKBT),
                ImmersionHeaterModelPart::from_time_points((time(03, 30, 00), 37.0), (time(04, 20, 00), 55.0), Sensor::TKBT),
            ]
        );
    ...
    fn check_deserialization() {
        ...
        let parts = vec![
            ImmersionHeaterModelPart::from_time_points((time(02, 10, 00), 30.0), (time(04, 05, 00), 50.0), Sensor::TKBT),
            ImmersionHeaterModelPart::from_time_points((time(00, 30, 00), 25.6), (time(01, 30, 00), 50.3), Sensor::TKTP),
        ];
